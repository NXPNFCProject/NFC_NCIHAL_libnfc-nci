...
#if (NXP_EXTNS == TRUE)
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "technology_mask:<0x%x>/<0x%x>/<0x%x>",
      technologies_screen_lock, technologies_screen_off,
      technologies_screen_off_lock);
#endif
...
#if (NXP_EXTNS == TRUE)
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "protocol_mask:<0x%x>/<0x%x>/<0x%x>",
      protocols_screen_lock, protocols_screen_off,
