...
tNFA_STATUS nfa_t4tnfcee_exec_file_operation() {
  ...
  if (status != NFA_STATUS_OK) {
    LOG(VERBOSE) << StringPrintf("%s T4T info not able to set. Return", __func__);
    return status;
  }
  status = RW_T4tNfceeSelectApplication();
  if (status != NFA_STATUS_OK) {
    LOG(VERBOSE) << StringPrintf("%s T4T Select application failed", __func__);
    return status;
  } else {
...
bool nfa_t4tnfcee_handle_op_req(tNFA_T4TNFCEE_MSG* p_data) {
  LOG(VERBOSE) << StringPrintf("nfa_t4tnfcee_handle_op_req: op=0x%02x",
                             p_data->op_req.op);
  ...
  switch (p_data->op_req.op) {
    ...
    case NFA_T4TNFCEE_OP_READ: {
      if (!is_read_precondition_valid(p_data)) {
        LOG(VERBOSE) << StringPrintf("%s Failed", __func__);
...
static void nfa_t4tnfcee_check_sw(tRW_DATA* p_rwData) {
  ...
  if ((status_words != T4T_RSP_CMD_CMPLTED) &&
    ...
    LOG(VERBOSE) << StringPrintf("status 0x%X", status_words);
  }
}
/*******************************************************************************
 **
...
void nfa_t4tnfcee_handle_t4t_evt(tRW_EVENT event, tRW_DATA* p_rwData) {
  LOG(VERBOSE) << StringPrintf("%s: Enter event=0x%02x 0x%02x", __func__, event,
                             p_rwData->status);
  switch (event) {
    case RW_T4T_RAW_FRAME_EVT:
      nfa_t4tnfcee_check_sw(p_rwData);
      LOG(VERBOSE) << StringPrintf("%s RW_T4T_RAW_FRAME_EVT", __func__);
      nfa_t4tnfcee_handle_file_operations(p_rwData);
      break;
    case RW_T4T_INTF_ERROR_EVT:
      LOG(VERBOSE) << StringPrintf("%s RW_T4T_INTF_ERROR_EVT", __func__);
      nfa_t4tnfcee_handle_file_operations(p_rwData);
      break;
    default:
...
void nfa_t4tnfcee_store_cc_info(NFC_HDR* p_data) {
  ...
  if (NULL != p_data) {
    ...
    LOG(VERBOSE) << StringPrintf("%s empty cc info", __func__);
    return;
  }
...
void nfa_t4tnfcee_store_rx_buf(NFC_HDR* p_data) {
  uint8_t* p;
  LOG(VERBOSE) << StringPrintf("%s copying data len %d  rd_offset: %d", __func__,
  ...
  if (NULL != p_data) {
    ...
    LOG(VERBOSE) << StringPrintf("%s Data is NULL", __func__);
  }
}
...
void nfa_t4tnfcee_handle_file_operations(tRW_DATA* p_rwData) {
  ...
  LOG(VERBOSE) << StringPrintf("%s currState : 0x%02x", __func__,
                             nfa_t4tnfcee_cb.prop_rw_state);
  switch (nfa_t4tnfcee_cb.prop_rw_state) {
    ...
    case WAIT_READ_CC_FILE: {
      ...
      if (ccFileInfo.find(nfa_t4tnfcee_cb.cur_fileId) == ccFileInfo.end()) {
        LOG(VERBOSE) << StringPrintf("%s FileId Not found in CC", __func__);
...
bool isReadPermitted(void) {
  if (ccFileInfo.find(nfa_t4tnfcee_cb.cur_fileId) == ccFileInfo.end()) {
    LOG(VERBOSE) << StringPrintf("%s FileId Not found", __func__);
    return false;
  }
...
uint16_t nfa_t4tnfcee_get_len(tRW_DATA* p_rwData) {
  ...
  if (readLen > 0x00) {
    LOG(VERBOSE) << StringPrintf("%s readLen  0x%x", __func__, readLen);
  } else {
    LOG(VERBOSE) << StringPrintf("%s No Data to Read", __func__);
  }
  return readLen;
}
