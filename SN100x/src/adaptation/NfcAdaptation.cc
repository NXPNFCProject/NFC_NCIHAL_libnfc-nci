...
static void HalGetProperty_cb(::android::hardware::hidl_string value)
{
  ...
  if(NfcAdaptation::GetInstance().propVal.size()){
    DLOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("%s: received value -> %s",
            __func__,NfcAdaptation::GetInstance().propVal.c_str());
  } else {
    DLOG_IF(INFO, nfc_debug_enabled)
 ...
 **
 ** Description      It shall be called from libnfc-nci to set the value of given
 **                  key in HAL context.
 **
