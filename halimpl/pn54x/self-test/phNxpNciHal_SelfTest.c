...
static nci_test_data_t swp2_test_data[10] = {
    {{
...
static nci_test_data_t swp1_test_data[10] = {
...
static nci_test_data_t download_pin_test_data1[10] = {
    {{
      0x04, {0x20, 0x00, 0x01, 0x00} /* cmd */
     },
     {
...
static nci_test_data_t download_pin_test_data2[10] = {
    {{
  ...
  if (hal_write_timer_fired == 1) {
    ...
    if(pInfo->pBuff[0] == NCI_MT_RSP && ((pInfo->pBuff[1] & NCI_OID_MASK) == NCI_MSG_CORE_RESET)){
      ...
      mSelfTestHdlr.nci_version = pInfo->pBuff[5];
        ...
        if(mSelfTestHdlr.nci_version == pInfo->pBuff[4]){
...
NFCSTATUS phNxpNciHal_initialize_chipType()
{
    ...
    if(!phNxpNciHal_writeLocked(pCoreResetCmd)){
      ...
      NXPLOG_NCIHAL_E("Response not received for CORE RESET command");
      return NFCSTATUS_FAILED;
    }
...
//    [NCI] -> [0x2F 0x30 0x04 0x00 0x00 0x01 0xFF]
  ...
  prbs_test_data[0].exp_rsp.p_data[4] = nfcFL.platformFL._NFCC_RESET_RSP_LEN;
  prbs_test_data[1].exp_rsp.p_data[2] = NFCC_EXP_RES_DATA_BYTE2;
  if(nfcFL.chipType == pn557){
      prbs_test_data[1].cmd.len = NFCC_CMD_NCI20_LEN;
      memcpy(prbs_test_data[1].cmd.p_data, nfcc_core_init_nci20_cmd, NFCC_CMD_NCI20_LEN);
  }else{
      prbs_test_data[1].cmd.len = NFCC_CMD_NCI10_LEN;
      memcpy(prbs_test_data[1].cmd.p_data, nfcc_core_init_nci10_cmd, NFCC_CMD_NCI10_LEN);
  }

  if(nfcFL.chipType == pn547C2)
  {
      prbs_test_data[2].cmd.len = sizeof(nfcc_power_mgt_cmd);
      memcpy(prbs_test_data[2].cmd.p_data, nfcc_power_mgt_cmd, prbs_test_data[2].cmd.len);
      prbs_test_data[2].exp_rsp.len = sizeof(nfcc_power_mgt_rsp);
      memcpy(prbs_test_data[2].exp_rsp.p_data, nfcc_power_mgt_rsp, prbs_test_data[2].cmd.len);
      prbs_test_data[2].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
      prbs_test_data[2].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
      prbs_test_data[2].rsp_validator = st_validator_testEquals;
      prbs_test_data[2].ntf_validator = st_validator_null;
  }
...
NFCSTATUS phNxpNciHal_RfFieldTest(uint8_t on) {
  ...
  if (on == 0x01) {
    rf_field_on_test_data[i].exp_rsp.p_data[4] = nfcFL.platformFL._NFCC_RESET_RSP_LEN;
    i++;// i val 1
    rf_field_on_test_data[i].exp_rsp.p_data[2] = NFCC_EXP_RES_DATA_BYTE2;
    if(nfcFL.chipType == pn557){
        rf_field_on_test_data[i].cmd.len = NFCC_CMD_NCI20_LEN;
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_core_init_nci20_cmd, NFCC_CMD_NCI20_LEN);
    }else{
        rf_field_on_test_data[i].cmd.len = NFCC_CMD_NCI10_LEN;
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_core_init_nci10_cmd, NFCC_CMD_NCI10_LEN);
    }
    i++; //i val 2
    if(nfcFL.chipType != pn547C2)
    {
        rf_field_on_test_data[i].cmd.len = sizeof(nfcc_act_cmd);
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_act_cmd, rf_field_on_test_data[i].cmd.len); //Crashing
        rf_field_on_test_data[i].exp_rsp.len = sizeof(nfcc_act_exp_rsp);
        memcpy(rf_field_on_test_data[i].exp_rsp.p_data, nfcc_act_exp_rsp, rf_field_on_test_data[i].cmd.len);
        rf_field_on_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
        rf_field_on_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
        rf_field_on_test_data[i].rsp_validator = st_validator_testEquals;
        rf_field_on_test_data[i].ntf_validator = st_validator_null;
        i++; //i val 3

        rf_field_on_test_data[i].cmd.len = sizeof(nfcc_power_mgt_cmd);
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_power_mgt_cmd, rf_field_on_test_data[i].cmd.len);
        rf_field_on_test_data[i].exp_rsp.len = sizeof(nfcc_power_mgt_rsp);
        memcpy(rf_field_on_test_data[i].exp_rsp.p_data, nfcc_power_mgt_rsp, rf_field_on_test_data[i].cmd.len);
        rf_field_on_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
        rf_field_on_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
        rf_field_on_test_data[i].rsp_validator = st_validator_testEquals;
        rf_field_on_test_data[i].ntf_validator = st_validator_null;
    ...
    if(nfcFL.chipType != pn547C2){
        rf_field_on_test_data[i].cmd.len = sizeof(nfcc_antena_cmd1);
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_antena_cmd1, rf_field_on_test_data[i].cmd.len);
    }
    else{
        rf_field_on_test_data[i].cmd.len = sizeof(nfcc_antena_cmd2);
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_antena_cmd2, rf_field_on_test_data[i].cmd.len);
    }
    rf_field_on_test_data[i].exp_rsp.len = sizeof(nfcc_antena_rsp1);
    memcpy(rf_field_on_test_data[i].exp_rsp.p_data, nfcc_antena_rsp1, rf_field_on_test_data[i].cmd.len);
    rf_field_on_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
    rf_field_on_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
    rf_field_on_test_data[i].rsp_validator = st_validator_testEquals;
    rf_field_on_test_data[i].ntf_validator = st_validator_null;
    ...
    if(nfcFL.chipType != pn547C2)
    {
        rf_field_on_test_data[i].cmd.len = sizeof(nfcc_power_mgt_cmd_1);
        memcpy(rf_field_on_test_data[i].cmd.p_data, nfcc_power_mgt_cmd_1, rf_field_on_test_data[i].cmd.len);
        rf_field_on_test_data[i].exp_rsp.len = sizeof(nfcc_power_mgt_rsp);
        memcpy(rf_field_on_test_data[i].exp_rsp.p_data, nfcc_power_mgt_rsp, rf_field_on_test_data[i].cmd.len);
        rf_field_on_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
        rf_field_on_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
    ...
    rf_field_off_test_data[i].exp_rsp.p_data[4] = nfcFL.platformFL._NFCC_RESET_RSP_LEN;
    i++;//i val 1
    rf_field_off_test_data[i].exp_rsp.p_data[2] = NFCC_EXP_RES_DATA_BYTE2;
    if(nfcFL.chipType == pn557){
        rf_field_off_test_data[i].cmd.len = NFCC_CMD_NCI20_LEN;
        memcpy(rf_field_off_test_data[i].cmd.p_data, nfcc_core_init_nci20_cmd, NFCC_CMD_NCI20_LEN);
    }else{
        rf_field_off_test_data[i].cmd.len = NFCC_CMD_NCI10_LEN;
        memcpy(rf_field_off_test_data[i].cmd.p_data, nfcc_core_init_nci10_cmd, NFCC_CMD_NCI10_LEN);
    }
    i++;//i val 2
    if(nfcFL.chipType != pn547C2)
    {
        rf_field_off_test_data[i].cmd.len = sizeof(nfcc_act_cmd);
        memcpy(rf_field_off_test_data[i].cmd.p_data, nfcc_act_cmd, rf_field_off_test_data[i].cmd.len);
        rf_field_off_test_data[i].exp_rsp.len = sizeof(nfcc_act_exp_rsp);
        memcpy(rf_field_off_test_data[i].exp_rsp.p_data, nfcc_act_exp_rsp, rf_field_off_test_data[i].cmd.len);
        rf_field_off_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
        rf_field_off_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
        rf_field_off_test_data[i].rsp_validator = st_validator_testEquals;
        rf_field_off_test_data[i].ntf_validator = st_validator_null;
        i++; //i val 3

        rf_field_off_test_data[i].cmd.len = sizeof(nfcc_power_mgt_cmd);
        memcpy(rf_field_off_test_data[i].cmd.p_data, nfcc_power_mgt_cmd, rf_field_off_test_data[i].cmd.len);
        rf_field_off_test_data[i].exp_rsp.len = sizeof(nfcc_power_mgt_rsp);
        memcpy(rf_field_off_test_data[i].exp_rsp.p_data, nfcc_power_mgt_rsp, rf_field_off_test_data[i].cmd.len);
        rf_field_off_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
        rf_field_off_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
        rf_field_off_test_data[i].rsp_validator = st_validator_testEquals;
        rf_field_off_test_data[i].ntf_validator = st_validator_null;
    ...
    if(nfcFL.chipType != pn547C2){
        rf_field_off_test_data[i].cmd.len = sizeof(nfcc_antena_cmd3);
        memcpy(rf_field_off_test_data[i].cmd.p_data, nfcc_antena_cmd3, rf_field_off_test_data[i].cmd.len);
    }
    else{
        rf_field_off_test_data[i].cmd.len = sizeof(nfcc_antena_cmd4);
        memcpy(rf_field_off_test_data[i].cmd.p_data, nfcc_antena_cmd4, rf_field_off_test_data[i].cmd.len);
    }
    rf_field_off_test_data[i].exp_rsp.len = sizeof(nfcc_antena_rsp1);
    memcpy(rf_field_off_test_data[i].exp_rsp.p_data, nfcc_antena_rsp1, rf_field_off_test_data[i].cmd.len);
    rf_field_off_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
    rf_field_off_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
    rf_field_off_test_data[i].rsp_validator = st_validator_testEquals;
    rf_field_off_test_data[i].ntf_validator = st_validator_null;
...
NFCSTATUS phNxpNciHal_AntennaSelfTest(phAntenna_St_Resp_t* phAntenna_St_Resp) {
  ...
  antenna_self_test_data[i].exp_rsp.p_data[4] = nfcFL.platformFL._NFCC_RESET_RSP_LEN;
  i++;//i val 1
  antenna_self_test_data[i].exp_rsp.p_data[2] = NFCC_EXP_RES_DATA_BYTE2;
  if(nfcFL.chipType == pn557){
      antenna_self_test_data[i].cmd.len = NFCC_CMD_NCI20_LEN;
      memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_core_init_nci20_cmd, NFCC_CMD_NCI20_LEN);
  }else{
      antenna_self_test_data[i].cmd.len = NFCC_CMD_NCI10_LEN;
      memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_core_init_nci10_cmd, NFCC_CMD_NCI10_LEN);
  }
  i+=2;//i val 3, By default structure will be filled till array 3

  if(nfcFL.chipType != pn547C2)
  {
      antenna_self_test_data[i].cmd.len = sizeof(nfcc_power_mgt_cmd);
      memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_power_mgt_cmd, antenna_self_test_data[i].cmd.len);
      antenna_self_test_data[i].exp_rsp.len = sizeof(nfcc_power_mgt_rsp);
      memcpy(antenna_self_test_data[i].exp_rsp.p_data, nfcc_power_mgt_rsp, antenna_self_test_data[i].cmd.len);
      antenna_self_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
      antenna_self_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
      antenna_self_test_data[i].rsp_validator = st_validator_testEquals;
      antenna_self_test_data[i].ntf_validator = st_validator_null;
      i++; //i val 4
  }
  antenna_self_test_data[i].cmd.len = sizeof(nfcc_antena_cmd5);
  memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_antena_cmd5, antenna_self_test_data[i].cmd.len);
  antenna_self_test_data[i].exp_rsp.len = sizeof(nfcc_antena_rsp2);
  memcpy(antenna_self_test_data[i].exp_rsp.p_data, nfcc_antena_rsp2, antenna_self_test_data[i].cmd.len);
  antenna_self_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
  antenna_self_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
  antenna_self_test_data[i].rsp_validator = st_validator_testAntenna_Txldo;
  antenna_self_test_data[i].ntf_validator = st_validator_null;
  i++; //i val 5
  if(nfcFL.chipType != pn547C2){
      antenna_self_test_data[i].cmd.len = sizeof(nfcc_antena_cmd7);
      memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_antena_cmd7, antenna_self_test_data[i].cmd.len);
  }
  else{
      antenna_self_test_data[i].cmd.len = sizeof(nfcc_antena_cmd8);
      memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_antena_cmd8, antenna_self_test_data[i].cmd.len);
  }
  antenna_self_test_data[i].exp_rsp.len = sizeof(nfcc_antena_rsp2);
  memcpy(antenna_self_test_data[i].exp_rsp.p_data, nfcc_antena_rsp2, antenna_self_test_data[i].cmd.len);
  antenna_self_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
  antenna_self_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
  antenna_self_test_data[i].rsp_validator = st_validator_testAntenna_AgcVal;
  antenna_self_test_data[i].ntf_validator = st_validator_null;
  i++;//i val 6
  antenna_self_test_data[i].cmd.len = sizeof(nfcc_antena_cmd5);
  memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_antena_cmd5, antenna_self_test_data[i].cmd.len);
  ...
  antenna_self_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
  antenna_self_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
  antenna_self_test_data[i].rsp_validator = st_validator_testAntenna_AgcVal_FixedNfcLd;
  antenna_self_test_data[i].ntf_validator = st_validator_null;
  i++;//i val 7

  if(nfcFL.nfccFL._HW_ANTENNA_LOOP4_SELF_TEST){
    antenna_self_test_data[i].cmd.len = sizeof(nfcc_antena_cmd6);
    memcpy(antenna_self_test_data[i].cmd.p_data, nfcc_antena_cmd6, antenna_self_test_data[i].cmd.len);
    antenna_self_test_data[i].exp_rsp.len = sizeof(nfcc_antena_rsp2);
    memcpy(antenna_self_test_data[i].exp_rsp.p_data, nfcc_antena_rsp2, antenna_self_test_data[i].cmd.len);
    antenna_self_test_data[i].exp_ntf.len = NFCC_EXP_NTF_LEN_NULL;
    antenna_self_test_data[i].exp_ntf.p_data[0] = NFCC_EXP_NTF_DATA_NULL;
